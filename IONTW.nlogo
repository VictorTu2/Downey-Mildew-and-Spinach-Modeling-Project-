  ...
  if (count turtles > 0) [
    ...
    if (set-state-by = "Number of nodes") [
      ...
      if (count candidates > 0) [
        ask n-of (min list num/frac count candidates) candidates [ ifelse (set-state-to = "Infectious") [ become-infectious ] [ become-removed ] ]
      ]
    ]
    if (set-state-by = "Fraction of nodes") [
      ...
      let candidates turtles with [susceptible? and (min-deg <= count link-neighbors)]
      if (count candidates > 0) [
        ask n-of (min list round((count candidates) * num/frac) count candidates) candidates [ ifelse (set-state-to = "Infectious") [ become-infectious ] [ become-removed ] ]
      ]
    ]
    if (set-state-by = "Vector from input") [
      ...
      let candidates turtle-set map [ ?1 -> turtle ?1 ] node-list
      if (count candidates > 0) [
...
to-report is-realizable [degrees]
  ...
  let sum-d butfirst reduce [ [?1 ?2] -> lput (?2 + last ?1) ?1 ] fput [0] degrees ;; Cumulative sum
  let realizable (last sum-d mod 2) = 0
  if realizable [
    let k 1
    while [realizable and k <= (length degrees)] [
      let min-d sum map [ ?1 -> min list (item ?1 degrees) k ] n-values ((length degrees) - k) [ ?1 -> ?1 + k ]
...
to setup-regular-tree
  ...
  while [cnt <= floor(lambda)] [
    foreach roots [ ?1 ->
      ...
      ask ?1 [create-links-with leaves]
...
to setup-spatially-clustered-network
  ...
  if ((lambda < 0)  or (lambda > 1)) [
    user-message "Edge density (lambda) must be a number between 0 and 1."
    stop
  ]

  let num-links round(lambda * (num-nodes * (num-nodes - 1) / 2))
...
to setup-nne2
  ...
  let m last filter [ ?1 -> remainder num-nodes ?1 = 0 ] n-values floor(sqrt(num-nodes)) [ ?1 -> ?1 + 1 ]
  let n num-nodes / m
...
to set-degrees-from-distribution [f]
  ...
  let cum-f butfirst reduce [ [?1 ?2] -> lput (?2 + last ?1) ?1 ] fput [0] f ;; Cumulative sum
  let rand 0
  let i 1
  while [i <= num-nodes] [
    set rand random-float (last cum-f)
    ask turtle (i - 1) [ set degree (length (filter [ ?1 -> ?1 < rand ] cum-f)) ]
    set i i + 1
  ]
end
...
to setup-generic-scale-free
  ...
  let nodes setup-nodes num-nodes
  layout-circle (sort turtles) max-pxcor - 1
  set distribution fput 0 map [ ?1 -> (1 / ?1) ^ lambda ] n-values (num-nodes - 1) [ ?1 -> 1 + ?1 ]
  set-degrees-from-distribution distribution
  ifelse is-realizable [degree] of turtles [
    havel-hakimi
    randomize-edges []
...
to setup-pref-attach
  if (d < 1 or lambda < 1 or (d mod 1 != 0) or (lambda mod 1 != 0)) [
    clean
    user-message "The size of the initial complete graph (lambda) and the number of connections at each step (d) must be positive integers."
    stop
  ]
  if (lambda > num-nodes) [
    clean
  ...
  while [ctr < num-nodes]
  [
    let degrees map [ ?1 -> [degree] of ?1 ] sort turtles
    let cumdeg butfirst reduce [ [?1 ?2] -> lput (?2 + last ?1) ?1 ] fput [0] degrees
    ask setup-nodes 1 [
      let i 1
      set degree m
      while [i <= m] [
        let rand random-float (last cumdeg)
        let friend turtle length (filter [ ?1 -> ?1 < rand ] cumdeg)
...
to go
  ...
  [
    ...
    let rate-list ( sentence (n-values (length infectious-list) [end-infection-rate]) (n-values (length latent-list) [end-latency-rate]) (n-values (length removed-list) [lose-immunity-rate]) (map [ ?1 -> (count [link-neighbors with [infectious?]] of ?1) * infection-rate ] susceptible-list) )
    let cum-rate-list butfirst reduce [ [?1 ?2] -> lput (?2 + last ?1) ?1 ] fput [0] rate-list ;; Cumulative sum

    let rnum random-float (last cum-rate-list)

    set change-state-list (list item (length (filter [ ?1 -> ?1 < rnum ] cum-rate-list)) all-list)
...
to compute-shortest-paths
  ...
  let current nobody
  ask turtles [
    foreach sort turtles [ ?1 ->  ;; Workaround - turtles can't ask other turtles, so have to use foreach
      ask ?1 [
        set unvisited true
        set dist N
      ]
    ...
    set shortest-path map [ ?1 -> [dist] of ?1 ] last conncomp
  ]
  set conncomp sort-by [ [?1 ?2] -> length ?1 > length ?2 ] remove-duplicates conncomp
end
...
to output-parameters
  ...
  ifelse (N != 0) [
    ...
    let PL 0
    if (N0 > 1) [
      set PL ((reduce + map [ ?1 -> sum([shortest-path] of ?1) ] myturtles) / (N0 * (N0 - 1)))
    ]
  ...
  ifelse (N != 0) [
    ...
    let spaths []
    ask myturtles [
      set spaths (sentence spaths filter [ ?1 -> ?1 > 0 ] shortest-path)
    ]
    let diam 0
    if (not empty? spaths) [
      set diam max(spaths)
...
to plot-degree-distribution
  if (count turtles > 0) [
    let bins n-values (1 + max [count link-neighbors] of turtles) [ ?1 -> ?1 ]
    let hist map [ ?1 -> count turtles with [count link-neighbors = ?1] ] bins
...
to plot-clustering-coefficients [normalized]
  ...
  if (N > 1) [ ; Need at least two nodes for this to make sense
    ...
    let bins n-values nbins [ ?1 -> min(coeffs) + ?1 * step ]
    let hist map [ ?1 -> count turtles with [(is-number? clustering) and (?1 <= clustering) and (clustering < (?1 + step))] ] bins
...
to plot-shortest-paths
  if (count turtles != 0) [
    ...
    let bins n-values diam [ ?1 -> 1 + ?1 ]
    let hist []
    foreach bins [ ?1 ->
      let b ?1
      set hist lput length(filter [ ??1 -> ??1 = b ] spaths) hist
    ]
    if (not empty? bins) [
...
to plot-probability-distribution
  if not empty? distribution [
    let bins n-values length(distribution) [ ?1 -> ?1 ]
    let hist map [ ?1 -> ?1 / sum(distribution) ] distribution
    plot-histogram bins hist
  ]
end
...
to-report turtles-from-file [filename]
  ...
  file-close
  report turtle-set map [ ?1 -> turtle ?1 ] output
end
...
to load-from-file [filename]
  ...
  let itype remove " " file-read-line
  let nitype ""
  let nptype ""
  let items []
  let splitline []
  ...
  while [not file-at-end?] [
    if (not file-at-end? and not member? itype ["distribution" "degrees" "network" "parameters" ""]) [
      print (word "Error on line " lnum ": Expected a file type.  Got ''" itype "'' instead.")
      file-close
      stop
    ]
    if (itype = "distribution") [
      ...
      set distribution map [ ?1 -> ?1 / M ] distribution
    ...
    if (itype = "network") [
      ...
      while [not file-at-end? and (member? nitype ["edge" "state" "node" "set" ""] or simple)] [
        ...
        ifelse (not simple) [
          if (not file-at-end? and not member? nitype ["set" "node" "edge" "state" "" "distribution" "degrees" "network" "parameters"]) [
            print (word "Error on line " lnum ": Expected a network type.  Got ''" nitype "'' instead.")
            file-close
            stop
          ]
          if (nitype = "set") [
            ...
            if (not file-at-end? and not member? nptype ["num-nodes" "network-type" "lambda" "d"]) [
              print (word "Error on line " lnum ": Expected a network parameter type.  Got ''" nptype "'' instead.")
              file-close
              stop
            ]
            if (nptype = "num-nodes") [
            ...
            if (nptype = "network-type") [
              if (not file-at-end? and not member? item 1 splitline ["Complete Graph" "Empty Graph" "Erdos-Renyi" "Nearest-neighbor 1" "Nearest-neighbor 2" "Small World 1" "Small World 2" "Preferential Attachment" "Generic Scale-free" "Spatially Clustered" "Random Regular" "Regular Tree" "Custom Distribution"]) [
                print (word "Error on line " lnum ": Expected a network-type.  Got ''" item 1 splitline "'' instead.")
                file-close
                stop
              ]
      ...
      while [not file-at-end? and nitype = "set"] [
        ...
        if (nitype = "set") [
          ...
          if (nptype = "model-time") [
            if (not file-at-end? and not member? item 1 splitline ["Continuous" "Discrete"]) [
              print (word "Error on line " lnum ": Expected a model-time type.  Got ''" item 1 splitline "'' instead.")
              file-close
              stop
            ]
...
IONTW simulates both discrete and continuous time agent-based models of infectious disease dynamics on networks. It can simulate models of the following types:

 * SI (`gain-immunity` = Off, `latent-period` = Off, `end-infection-rate`/`prob` = 0),
 * SIS (`gain-immunity` = Off, `latent-period` = Off, `end-infection-rate`/`prob` > 0),
 * SIR (`gain-immunity` = On, `latent-period` = Off, `end-infection-rate`/`prob` > 0),
 * SEI (`gain-immunity` = Off, `latent-period` = On, `end-infection-rate`/`prob` = 0),
 * SEIS (`gain-immunity` = Off, `latent-period` = On, `end-infection-rate`/`prob` > 0),
 * SEIR (`gain-immunity` = On, `latent-period` = On, `end-infection-rate`/`prob` > 0),

where S, E, I, and R stand for susceptible, exposed (latent), infectious, and removed, respectively.  

In the discrete model, at each discrete time step, transitions between states are controlled by the `infection-prob` (S -> I or S -> E), `end-infection-prob` (I -> S or I -> R), and `end-latency-prob` (E -> I) parameters.  The continuous model is simulated based on the Gillespie’s algorithm, with the transitions between states controlled by the `infection-rate`, `end-infection-rate`, and `end-latency-rate` parameters.  The rate parameters correspond to the same transitions as stated above for the discrete probabilities.

The code can handle all network types specified in the `network-type` drop-down menu, with network parameters controlled by `number-of-nodes`, `lambda`, and `d`.  Note that not all of these parameters are used for each network type.

Batch runs for computing estimates over multiple replicates or parameter sweeps can be accomplished by clicking on `Tools`->`BehaviorSpace` and then creating a new experiment.  Multiple runs can be computed, with the output of desired measures sent to a table or spreadsheet.  Analysis can then be done in your program of choice, such as Matlab or Excel.

The Reference Guide that is included with the software package gives detailed instructions on how to enforce these and other options.
...
Copyright 2014 M. Drew LaMar, Winfried Just, and Hannah Callender

![GPL 3.0](http://www.gnu.org/graphics/gplv3-88x31.png)
...
@#$#@#$#@
<experiments>
  <experiment name="&quot;0&quot;-infectious_change_infprob_endinfprob_2species" repetitions="100" runMetricsEveryStep="false">
    <setup>last-init</setup>
    <go>go</go>
    <metric>count turtles with [removed?]</metric>
    <metric>ticks</metric>
    <enumeratedValueSet variable="spawn-kill">
      <value value="&quot;Spawn&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="gain-immunity">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="lambda">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="end-infection-rate">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="end-infection-prob">
      <value value="0.5"/>
      <value value="1"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="network-type">
      <value value="&quot;Nearest-neighbor 2&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="set-state-by">
      <value value="&quot;Vector from input&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="plot-metric">
      <value value="&quot;Degree Distribution&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="auto-set">
      <value value="true"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="min-deg">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="d">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="end-latency-rate">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="model-time">
      <value value="&quot;Discrete&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-nodes">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num/frac">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="set-state-to">
      <value value="&quot;Infectious&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="time-step">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="end-latency-prob">
      <value value="0.5"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="&quot;0&quot;-infectious_change_infprob_endinfprob_2species_table" repetitions="100" runMetricsEveryStep="false">
    <setup>last-init</setup>
    <go>go</go>
    <metric>count turtles with [removed?]</metric>
    <metric>ticks</metric>
  ...
  <experiment name="test_experiment_for_fun" repetitions="100" runMetricsEveryStep="true">
    <setup>last-init</setup>
    <go>go</go>
    <metric>count turtles [removed?]</metric>
    <metric>Ticks</metric>
    <enumeratedValueSet variable="spawn-kill">
      <value value="&quot;Spawn&quot;"/>
    </enumeratedValueSet>
